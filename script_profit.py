...
#    print(type(data[indep][0]))
    ...
    X = np.array(data[indep].astype(float))
...
#    theta = [0, 0]
#    print("main theta : " + str(theta))
    theta = fit(X, y, theta, 0.002, 1000)
    print("theta apres le fit" , theta)
    #theta, T_history, J_history = fit_with_cost(X, y, theta, 0.02, 100)
    visualizeRegression(X, y, theta)
    #visualizeCost(J_history)
    #visualizeTheta (T_history) 
    print("Prediction pour 5734: "+str(hipothesis(5734, theta)))
